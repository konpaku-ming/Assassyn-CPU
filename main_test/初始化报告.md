...
本报告描述了如何使用 `accumulate_data.bin` 和 `accumulate_text.bin` 文件初始化 Assassyn CPU 的主内存（main_memory）和指令缓存（icache），以及如何正确初始化栈指针（sp）以支持程序运行。
...
在运行 `accumulate` 程序时，发现栈指针 `sp`（寄存器 x2）没有被合理初始化，导致程序无法正确执行。

**问题根源**：
1. **accumulate 程序的第一条指令**是 `addi sp, sp, -32` (机器码: `0xfe010113`)
   - 这条指令假设 `sp` 已经指向一个有效的栈顶地址
   - 它尝试在栈上分配 32 字节的栈帧

2. **原始寄存器堆初始化**：在 `src/main.py` 中，寄存器堆没有指定初始值
   ```python
   # 原始代码（有问题）
   reg_file = RegArray(Bits(32), 32)  # 所有寄存器默认初始化为 0
   ```
   - 这导致 `sp` (x2) 的初始值为 0
   - 执行 `sp = sp - 32` 后，`sp = 0 - 32 = 0xffffffe0` (负数/非法地址)
   - 程序后续的栈操作（sw/lw）会访问非法内存地址，导致错误

3. **RISC-V ABI 要求**：
   - 根据 RISC-V 调用约定，栈指针 `sp` 应该在程序开始前指向一个有效的栈区域
   - 栈是向下增长的（从高地址向低地址）
   - 栈顶应该初始化为 RAM 的最高地址（或附近）

### 解决方案

**方案：在 CPU 复位时初始化栈指针**

在 `src/main.py` 的 `build_cpu()` 函数中，修改寄存器堆的初始化代码，显式地将 `sp` (x2) 初始化为栈顶地址。
...
1. **栈顶地址计算**：
   ...
   ```
   - `depth_log` 是内存深度的对数（默认为 16）
   - `1 << depth_log` = 2^16 = 65536 个字（words）
   - `(1 << depth_log) << 2` = 65536 * 4 = 262144 字节 = 0x40000
   - 这是 RAM 末尾之后的第一个地址，作为栈顶是合适的

2. **为什么栈顶在 RAM 之外？**
   - 栈向下增长（decrements），第一次使用前会先减去栈帧大小
   - 例如：`sp = 0x40000`，执行 `addi sp, sp, -32` 后，`sp = 0x3ffe0`
   - 这个地址在 RAM 范围内（0x00000000 - 0x0003ffff）

3. **寄存器初始化列表**：
   ```python
   reg_init = [0] * 32      # 所有寄存器初始化为 0
   reg_init[2] = stack_top   # x2 (sp) = 栈顶地址
   ```
...
│     (Available RAM)         │
│                             │
├─────────────────────────────┤ 约 0x00160 (accumulate 之后)
│      指令区域                │
│   (Instruction Region)      │
│   accumulate: 0x00-0x57     │
├─────────────────────────────┤ 0x00000 (程序起始)
│      保留区域                │
└─────────────────────────────┘

总容量：2^16 字 = 65536 字 = 256 KB (0x40000 字节)
```

### 验证方法

修改后，可以通过以下方式验证栈指针初始化是否正确：

#### 1. 检查初始化代码
查看 `src/main.py` 中的寄存器初始化部分，确认有类似代码：
```python
stack_top = (1 << depth_log) << 2
reg_init = [0] * 32
reg_init[2] = stack_top
reg_file = RegArray(Bits(32), 32, initializer=reg_init)
```
...
程序应该能够正常执行，而不会因为栈指针错误而崩溃。

#### 3. 检查仿真日志
在仿真输出中，可以查看：
- 初始 `sp` 值应该是 `0x40000` (262144)
- 第一条指令执行后，`sp` 应该变为 `0x3ffe0` (262112)
- 后续的栈操作（sw/lw）应该访问的是有效的内存地址

#### 4. 预期结果
- ✅ 程序能够成功初始化栈帧
- ✅ 局部变量（sum, i）能够正确存储和读取
- ✅ 程序最终计算出正确结果：5050 (0x13ba)

### 其他考虑

#### 1. 不同内存大小的适配
代码使用 `(1 << depth_log) << 2` 自动计算栈顶，因此适配任何内存大小：
- `depth_log=14` → 栈顶 = 0x10000 (64 KB)
- `depth_log=16` → 栈顶 = 0x40000 (256 KB)
- `depth_log=18` → 栈顶 = 0x100000 (1 MB)

#### 2. 为什么不在指令中初始化？
有些系统使用启动代码（如 `crt0.S`）在程序开始前设置 `sp`：
```assembly
la sp, _stack_top    # 从链接器脚本获取栈顶符号
```
...
这种方法更类似于嵌入式系统的做法（如 ARM Cortex-M），在复位向量中直接设置初始 `sp`。

#### 3. RISC-V ABI 兼容性
修改后的实现符合 RISC-V ABI 规范：
- x0 (zero) 固定为 0
- x2 (sp) 在程序入口前已初始化
- 其他寄存器可以是未定义值（我们初始化为 0）

### 总结

**问题**：栈指针 `sp` 未初始化，导致 accumulate 程序无法正常运行。

**原因**：寄存器堆默认全部初始化为 0，而程序假设 `sp` 已经指向有效栈顶。

**解决方案**：在 `src/main.py` 中显式初始化 `sp` (x2) 为栈顶地址 `(1 << depth_log) << 2`。

**修改位置**：`src/main.py` 第 96-103 行（原第 97 行）

**测试结果**：✅ 修改后，accumulate 程序能够正确执行，栈操作正常。
