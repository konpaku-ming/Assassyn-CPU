...
Radix-4 每次确定 2 位商；32 位除法共 16 轮（约 18 周期，含启动/结束）。下表展示前几轮的寄存器变化（rem：部分余数，q_acc：当前已确定的商高位）。

| 周期 | EX 阶段状态 | 关键寄存器（简化） | 说明 |
|------|-------------|--------------------|------|
| N    | start_divide() 触发，dividend=14，divisor=3，进入 DIV_PRE | rem=0，q_acc=0 | 初始化 |
| N+1  | 第 1 轮（处理 bits 31-30 → 实例中视作“00”） | rem=(0<<2)+0=0，试减失败 | 商两位=00，q_acc=0b00 |
| N+2  | 第 2 轮（示例抽取“11”） | rem=(0<<2)+3=3，3≥3 → rem=0 | 商两位=01，q_acc=0b01 |
| N+3  | 第 3 轮（示例抽取“10”） | rem=(0<<2)+2=2，2<3 → rem=2 | 商两位=00，q_acc=0b0100 |
| N+4  | 第 4 轮（示例抽取“00”） | rem=(2<<2)+0=8，8≥3 → rem=5 | 商两位=01，q_acc=0b0101_01 |
| …    | 剩余 12 轮按相同规则更新 rem/q_acc | … | … |
| N+17 | 最后一轮完成，进入 DIV_END | q_acc=0b0100 (十进制 4)，rem=0b10 (十进制 2) | quotient_ready=1 |

最终得到商=4，余数=2。完整 32 位流程同理，每轮处理 2 位商并更新 rem，直至生成最终商/余数。
