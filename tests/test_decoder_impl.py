...
class Driver(Module):
    ...
    def build(
        ...
        vectors = [
            ...
            (Bits(32)(0x00000098), Bits(32)(0x003100b3),
             Bits(32)(0x00000002), Bits(32)(0x00000003),
             Bits(4)(0x2), Bits(4)(0x3), Bits(1)(1), Bits(32)(0)),
            
            # 流水线刷新测试
            # add x1, x2, x3 + branch_target = 0x100
            (Bits(32)(0x0000009C), Bits(32)(0x003100b3),
             Bits(32)(0x00000002), Bits(32)(0x00000003),
             Bits(4)(0x2), Bits(4)(0x3), Bits(1)(0), Bits(32)(0x100)),
        ]
...
def check(raw_output):
    ...
    for line in lines:
        ...
        if "ID_Impl:" in line:
            # 尝试提取DecoderImpl输出信息
            impl_match = re.search(r"ID_Impl:.*?flush_if=(\w+).*?nop_if=(\w+).*?final_rd=(0x\w+)", line)
            if impl_match:
                # 找到最近的Driver idx
    ...
    for idx in sorted(decoder_impl_outputs.keys()):
        ...
        test_result = {
            ...
            'expected': {
                'flush_if': 'True' if int(driver_info['branch_target'], 16) != 0 else 'False',
                'nop_if': 'True' if (int(driver_info['branch_target'], 16) != 0 or 
                                   int(driver_info['stall_if'], 16) != 0) else 'False',
                'final_rd': '0x0' if (int(driver_info['branch_target'], 16) != 0 or 
                                      int(driver_info['stall_if'], 16) != 0) else '0x1'
            },
        ...
        if decoder_impl_info:
            # 验证flush_if
            flush_if_match = decoder_impl_info['flush_if'] == test_result['expected']['flush_if']
            
            # 验证nop_if
            nop_if_match = decoder_impl_info['nop_if'] == test_result['expected']['nop_if']
            
            # 验证final_rd
            final_rd_match = decoder_impl_info['final_rd'] == test_result['expected']['final_rd']
            ...
            if test_result['passed']:
                ...
                if not flush_if_match:
                    print(f"   flush_if不匹配: 预期 {test_result['expected']['flush_if']}, 实际 {decoder_impl_info['flush_if']}")
                if not nop_if_match:
                    print(f"   nop_if不匹配: 预期 {test_result['expected']['nop_if']}, 实际 {decoder_impl_info['nop_if']}")
                if not final_rd_match:
                    print(f"   final_rd不匹配: 预期 {test_result['expected']['final_rd']}, 实际 {decoder_impl_info['final_rd']}")
        else:
...
def generate_test_report(test_results):
    ...
    for instr, stats in instruction_stats.items():
        ...
        print(f"{instr:20} | 通过: {stats['passed']:2d}/{stats['total']:2d} | 通过率: {pass_rate:5.1f}%")
        ...
        for test in failed_tests:
            ...
            if test['actual'] and 'details' in test:
                ...
                if not details['flush_if_match']:
                    print(f"  - flush_if不匹配: 预期 {test['expected']['flush_if']}, 实际 {test['actual']['flush_if']}")
                if not details['nop_if_match']:
                    print(f"  - nop_if不匹配: 预期 {test['expected']['nop_if']}, 实际 {test['actual']['nop_if']}")
                if not details['final_rd_match']:
                    print(f"  - final_rd不匹配: 预期 {test['expected']['final_rd']}, 实际 {test['actual']['final_rd']}")
            else:
...
if __name__ == "__main__":
    print("="*80)
    print("MyCPU DecoderImpl模块测试")
    print("="*80)
    print("测试说明：")
    print("1. 本测试验证DecoderImpl模块是否能正确处理流水线控制信号")
    print("2. 测试覆盖流水线停顿(stall_if)和刷新(flush_if)信号的处理")
    print("3. 验证在流水线控制信号下，final_rd的正确生成")
    print("4. 在apptainer容器中运行测试:")
    print("   - source setup.sh")
    print("   - cd MyCPU")
    print("   - python tests/test_decoder_impl.py")
    print("="*80)
