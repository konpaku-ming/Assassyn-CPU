...
- **周期 1（IDLE → 特判分流）**
  - `tick` 在 `state=IDLE` 且 `valid_in=1` 时检查：
    ...
    - `divisor=1` → 进入 `DIV_1`。
    - 其他情况：进入 `DIV_PRE`，同时若为有符号除法提取操作数符号、取绝对值并记录 `div_sign`、`sign_r`。
- **周期 2（DIV_PRE：预处理）**
...
- **周期 3~18（DIV_WORKING：迭代 16 次，每次求 2 位商）**
  - 每个周期完成一轮：
    ...
    3. 按优先级选择最大可减倍数，更新 `remainder`，生成本轮商位 `q_bits`（00/01/10/11），合并回 `quotient` 低 2 位。
