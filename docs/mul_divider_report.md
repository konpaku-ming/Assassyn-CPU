...
## Radix16Divider（Radix-16 除法器，带 On-the-Fly 转换，约 10 周期）

### On-the-Fly (OTF) 转换原理

On-the-Fly 转换是一种避免最终符号数字到二进制转换的优化技术。OTF 通过维护两个并行更新的累加器来消除这一开销：

- **Q**：当前商估计值（标准二进制表示）
- **QM**：Q 减 1（始终等于 Q - 1）

每次迭代的更新规则：
- 正商位 (q >= 0)：Q_new = (Q << 4) + q，QM_new = (Q << 4) + (q - 1)
- 负商位 (q < 0)：Q_new = (QM << 4) + (16 + q)
