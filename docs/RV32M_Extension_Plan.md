...
class ALUOp:
    ...
    DIV    = Bits(16)(0b0000100000000000)  # Bit 11 (复用，通过funct3区分)
    DIVU   = Bits(16)(0b0001000000000000)  # Bit 12
    REM    = Bits(16)(0b0010000000000000)  # Bit 13
    REMU   = Bits(16)(0b0100000000000000)  # Bit 14
```

**注意**: 由于Bits(16)的限制，需要重新设计独热码分配或扩展为Bits(32)。

**建议方案**: 扩展为 `Bits(32)`，为未来指令集扩展预留空间。
...
| 阶段二：指令真值表更新   | 0.5小时  | `instruction_table.py` 添加8条指令    |
| 阶段三：译码器增强       | 1小时    | `decoder.py` 支持funct7匹配           |
| 阶段四：执行单元扩展     | 2小时    | `execution.py` 实现乘除法逻辑         |
| 阶段五：测试用例开发     | 3小时    | 单元测试 + 集成测试 + 性能测试        |
| 阶段六：文档更新         | 1小时    | 模块文档 + M扩展专项文档              |
| **总计**                 | **8.5小时** | 完整的RV32IM支持                    |
