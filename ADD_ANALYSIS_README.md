...
使用 `verify_accumulation.py` 验证0to100累加程序的正确性：
...
# 查看帮助
python3 verify_accumulation.py --help
```

该工具会：
- 检查程序是否从内存加载了正确的值 (0, 1, 2, ..., 100)
- 验证累加结果是否等于5050
- 识别数据初始化问题
- 提供详细的根本原因分析
...
✅ **所有305个加法指令的操作数和结果完全正确**
- CPU的ALU加法功能正常工作
- 数据旁路（Data Forwarding）机制正确实现
- 没有发现任何操作数错误或计算错误

❌ **累加程序结果不正确**
- 期望累加结果：0 + 1 + 2 + ... + 100 = 5050
- 实际累加结果：0
- 原因：内存数组包含全0，而非0-100的序列
- CPU硬件工作正常，问题在于输入数据未正确初始化

这表明Assassyn CPU的加法器设计和实现是正确的，但测试数据需要正确初始化。
...
- **期望结果：** 0 + 1 + 2 + ... + 100 = 5050
- **实际结果：** 0（所有加载的内存值都是0）
- **根本原因：** 内存数组未正确初始化，包含全0而非0-100的序列
